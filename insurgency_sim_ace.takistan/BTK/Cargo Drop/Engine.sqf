...
if ((_Selected == "LoadCargo") && (_SelectedTransporterTypeS)) exitWith {
	...
	_UnloadAction = _Transporter addAction [("<t color=""#fadfbe"">" + ("Unload cargo") + "</t>"),"BTK\Cargo Drop\Engine.sqf",["UnloadCargo"], 7, false, true, "", "driver _target == player"];
...
if ((_Selected == "LoadCargo") && (_SelectedTransporterTypeL)) exitWith {
	...
	if (_Object isKindOf "Car") then {
		...
		if (_Object isKindOf "Misc_cargo_cont_net1") then {
		_Object attachTo [_Transporter,[0,0,-1.8]];
		_Object setDir 90;
