...
if (typename _unit == typename "") then {
	...
	if (random 250 > 247) then {
		// suicide bomber script
		nul = [_unit, WEST, 10, 250] execVM "common\server\misc\bomber_random.sqf";
		_unit addMPEventHandler ["mpkilled","_this execVM ""common\server\misc\bomberkilled.sqf"""];		
	} else {
		// modifier to cause player to get -5 if they kill civilian
		_unit addMPEventHandler ["mpkilled","_this execVM ""common\server\misc\civkilled.sqf"""];
	};
...
[(getpos _man select 0)  + random 100 - random 100,(getpos _man select 1) + random 100 - random 100,0];
