	...
	if(_available) then{
		...
		[_x,_cannonsetup,_requestor] spawn {
			...
			for "_i" from 1 to _nrshells do {
				...
				if(_artitype == "ACE_M34") then{ // DPICM
					[_bomb,_artitype,getPos _bomb] spawn {
						_bmb = _this select 0;
						_artype = _this select 1;
						_pos = _this select 2;
						for "_j" from 1 to 3 do {
							_clusterb = _artype createVehicle [(_pos select 0) + 30 - random 60,(_pos select 1) + 30 - random 60,15];
							sleep (0.5 + random 2);
						};
					};
				};				
				...
				if(_artitype == "G_40mm_Smoke") then{ // DPICM
					[_bomb,_artitype,getPos _bomb] spawn {
						_bmb = _this select 0;
						_artype = _this select 1;
						_pos = _this select 2;
						for "_j" from 1 to 8 do {
							_clusterb = _artype createVehicle [(_pos select 0) + 15 - random 30,(_pos select 1) + 15 - random 30,20];
							sleep (0.5 + random 2);
						};
					};
				};

				if(_artitype == "G_40mm_SmokeRed") then{ // DPICM
					[_bomb,_artitype,getPos _bomb] spawn {
						_bmb = _this select 0;
						_artype = _this select 1;
						_pos = _this select 2;
