...
handleHeal = { 	
	...
	[nil, _injured, "loc", rSPAWN, player, { 
		...
		nMedics     = nMedics - 1;
		if (alive player && animationState _healer in healAnims) then { player setDamage 0; bloodloss = 0;}; 
	}] call RE; 
...
initBloodloss = { 
	...
	while{ alive player && damage player > 0}do{ 
		...
		if (round(time - curTime) % bloodlossFreq == 0) then { 			
			...
			sleep random 2;
			_injurysound = InjurySounds select (random (count InjurySounds - 1));
			_body = player;
			_body say _injurysound;
			r_say = [_body, _injurysound];
