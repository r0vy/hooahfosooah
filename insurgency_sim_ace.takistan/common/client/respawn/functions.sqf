...
respawnSystem = {
	...
	while {isDead(player) && time > 30} do {	
		...
		if (_camPlayer == camPlayer && enterSpawn) then {
			...
			_ctrlText ctrlSetStructuredText parseText format[
				"<t color='%2' shadow='1' shadowColor='#000000'>Spawning in %1</t><br/><br/><t color='#FFFFFF' shadow='1' shadowColor='#000000'>Left Arrow: Previous unit<br/>Right Arrow: Next unit<br/></t>"
			, abs ceil _c, "#FFFFFF"]; 
	...
	if (!isNull camPlayer && alive camPlayer) then {
		...
		_pos = startLocation modelToWorld [-10,-13+(squadNumber(player)-1)*6+(unitNumber(player)-1)*1.2,0];
